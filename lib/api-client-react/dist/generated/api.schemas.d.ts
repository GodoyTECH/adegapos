/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * Adega Smart POS API
 * OpenAPI spec version: 0.1.0
 */
export interface HealthStatus {
    status: string;
}
export interface ErrorResponse {
    error: string;
    message?: string;
}
export interface MessageResponse {
    message: string;
}
export interface LoginBody {
    email: string;
    password: string;
}
export type UserRole = (typeof UserRole)[keyof typeof UserRole];
export declare const UserRole: {
    readonly admin: "admin";
    readonly manager: "manager";
    readonly cashier: "cashier";
};
export interface User {
    id: number;
    name: string;
    email: string;
    role: UserRole;
    active: boolean;
    createdAt: string;
}
export interface AuthResponse {
    user: User;
    token: string;
}
export type CreateUserBodyRole = (typeof CreateUserBodyRole)[keyof typeof CreateUserBodyRole];
export declare const CreateUserBodyRole: {
    readonly admin: "admin";
    readonly manager: "manager";
    readonly cashier: "cashier";
};
export interface CreateUserBody {
    name: string;
    email: string;
    password: string;
    role: CreateUserBodyRole;
}
export type UpdateUserBodyRole = (typeof UpdateUserBodyRole)[keyof typeof UpdateUserBodyRole];
export declare const UpdateUserBodyRole: {
    readonly admin: "admin";
    readonly manager: "manager";
    readonly cashier: "cashier";
};
export interface UpdateUserBody {
    name?: string;
    email?: string;
    password?: string;
    role?: UpdateUserBodyRole;
    active?: boolean;
}
export interface Category {
    id: number;
    name: string;
    description?: string | null;
    active: boolean;
    createdAt: string;
}
export interface CreateCategoryBody {
    name: string;
    description?: string;
    active?: boolean;
}
export type ProductProductType = (typeof ProductProductType)[keyof typeof ProductProductType];
export declare const ProductProductType: {
    readonly simple: "simple";
    readonly composite: "composite";
};
export interface Product {
    id: number;
    categoryId?: number | null;
    categoryName?: string | null;
    name: string;
    barcode?: string | null;
    productType: ProductProductType;
    costPrice?: number | null;
    salePrice: number;
    marginPercentage?: number | null;
    stockQuantity: number;
    minStock: number;
    unitType: string;
    imageUrl?: string | null;
    description?: string | null;
    preparationInstructions?: string | null;
    preparationTimeMinutes?: number | null;
    internalNotes?: string | null;
    active: boolean;
    createdAt: string;
}
export type CreateProductBodyProductType = (typeof CreateProductBodyProductType)[keyof typeof CreateProductBodyProductType];
export declare const CreateProductBodyProductType: {
    readonly simple: "simple";
    readonly composite: "composite";
};
export interface CreateProductBody {
    categoryId?: number;
    name: string;
    barcode?: string;
    productType: CreateProductBodyProductType;
    costPrice?: number;
    salePrice: number;
    stockQuantity: number;
    minStock: number;
    unitType: string;
    imageUrl?: string;
    description?: string;
    preparationInstructions?: string;
    preparationTimeMinutes?: number;
    internalNotes?: string;
    active?: boolean;
}
export interface CompositeItem {
    id: number;
    compositeProductId: number;
    ingredientProductId: number;
    ingredientName: string;
    ingredientStock: number;
    quantityUsed: number;
    unitType: string;
    notes?: string | null;
}
export interface CreateCompositeItemBody {
    ingredientProductId: number;
    quantityUsed: number;
    unitType: string;
    notes?: string;
}
export type SaleStatus = (typeof SaleStatus)[keyof typeof SaleStatus];
export declare const SaleStatus: {
    readonly completed: "completed";
    readonly cancelled: "cancelled";
    readonly refunded: "refunded";
    readonly pending: "pending";
};
export type PaymentMethod = (typeof PaymentMethod)[keyof typeof PaymentMethod];
export declare const PaymentMethod: {
    readonly cash: "cash";
    readonly pix: "pix";
    readonly debit_card: "debit_card";
    readonly credit_card: "credit_card";
    readonly other: "other";
};
export interface Payment {
    id: number;
    method: PaymentMethod;
    amount: number;
    status: string;
}
export interface Sale {
    id: number;
    userId: number;
    userName: string;
    cashSessionId?: number | null;
    subtotal: number;
    discount: number;
    total: number;
    status: SaleStatus;
    cancellationReason?: string | null;
    paymentMethods: Payment[];
    createdAt: string;
}
export interface SaleItem {
    id: number;
    productId: number;
    productNameSnapshot: string;
    quantity: number;
    unitPrice: number;
    total: number;
}
export type SaleDetail = Sale & {
    items: SaleItem[];
};
export interface CreateSaleItemBody {
    productId: number;
    quantity: number;
    unitPrice: number;
}
export type CreatePaymentBodyMethod = (typeof CreatePaymentBodyMethod)[keyof typeof CreatePaymentBodyMethod];
export declare const CreatePaymentBodyMethod: {
    readonly cash: "cash";
    readonly pix: "pix";
    readonly debit_card: "debit_card";
    readonly credit_card: "credit_card";
    readonly other: "other";
};
export interface CreatePaymentBody {
    method: CreatePaymentBodyMethod;
    amount: number;
}
export interface CreateSaleBody {
    cashSessionId?: number;
    discount?: number;
    items: CreateSaleItemBody[];
    payments: CreatePaymentBody[];
    notes?: string;
}
export interface CancelSaleBody {
    reason: string;
}
export type StockEntryStatus = (typeof StockEntryStatus)[keyof typeof StockEntryStatus];
export declare const StockEntryStatus: {
    readonly normal: "normal";
    readonly low: "low";
    readonly zero: "zero";
};
export interface StockEntry {
    productId: number;
    productName: string;
    categoryName?: string | null;
    stockQuantity: number;
    minStock: number;
    unitType: string;
    status: StockEntryStatus;
    lastMovementAt?: string | null;
}
export type StockMovementType = (typeof StockMovementType)[keyof typeof StockMovementType];
export declare const StockMovementType: {
    readonly entry: "entry";
    readonly sale: "sale";
    readonly sale_composite: "sale_composite";
    readonly manual_adjustment: "manual_adjustment";
    readonly loss: "loss";
    readonly breakage: "breakage";
    readonly expiration: "expiration";
    readonly return: "return";
    readonly sale_cancellation: "sale_cancellation";
};
export interface StockMovement {
    id: number;
    productId: number;
    productName: string;
    userId: number;
    userName: string;
    type: StockMovementType;
    quantity: number;
    previousQuantity: number;
    newQuantity: number;
    reason?: string | null;
    createdAt: string;
}
export type CreateStockMovementBodyType = (typeof CreateStockMovementBodyType)[keyof typeof CreateStockMovementBodyType];
export declare const CreateStockMovementBodyType: {
    readonly entry: "entry";
    readonly manual_adjustment: "manual_adjustment";
    readonly loss: "loss";
    readonly breakage: "breakage";
    readonly expiration: "expiration";
    readonly return: "return";
};
export interface CreateStockMovementBody {
    productId: number;
    type: CreateStockMovementBodyType;
    quantity: number;
    reason?: string;
}
export type CashSessionStatus = (typeof CashSessionStatus)[keyof typeof CashSessionStatus];
export declare const CashSessionStatus: {
    readonly open: "open";
    readonly closed: "closed";
};
export interface CashSession {
    id: number;
    userId: number;
    userName: string;
    openingAmount: number;
    expectedAmount?: number | null;
    closingAmount?: number | null;
    status: CashSessionStatus;
    openedAt: string;
    closedAt?: string | null;
    totalSales?: number | null;
    salesCount?: number | null;
}
export interface OpenCashBody {
    openingAmount: number;
}
export interface CloseCashBody {
    closingAmount: number;
}
export type CashMovementType = (typeof CashMovementType)[keyof typeof CashMovementType];
export declare const CashMovementType: {
    readonly withdrawal: "withdrawal";
    readonly supply: "supply";
};
export interface CashMovement {
    id: number;
    cashSessionId: number;
    userId: number;
    userName: string;
    type: CashMovementType;
    amount: number;
    reason: string;
    createdAt: string;
}
export type CreateCashMovementBodyType = (typeof CreateCashMovementBodyType)[keyof typeof CreateCashMovementBodyType];
export declare const CreateCashMovementBodyType: {
    readonly withdrawal: "withdrawal";
    readonly supply: "supply";
};
export interface CreateCashMovementBody {
    type: CreateCashMovementBodyType;
    amount: number;
    reason: string;
}
export interface DashboardSummary {
    revenueToday: number;
    salesCountToday: number;
    avgTicketToday: number;
    estimatedProfit: number;
    revenueYesterday: number;
    salesCountYesterday: number;
    avgTicketYesterday: number;
    lowStockCount: number;
    zeroStockCount: number;
}
export interface SalesByHour {
    hour: number;
    count: number;
    revenue: number;
}
export interface TopProduct {
    productId: number;
    productName: string;
    categoryName?: string | null;
    quantitySold: number;
    revenue: number;
}
export interface PaymentMethodSummary {
    method: string;
    count: number;
    total: number;
}
export interface CategorySales {
    categoryId?: number | null;
    categoryName: string;
    revenue: number;
    count: number;
}
export type LowStockItemStatus = (typeof LowStockItemStatus)[keyof typeof LowStockItemStatus];
export declare const LowStockItemStatus: {
    readonly low: "low";
    readonly zero: "zero";
};
export interface LowStockItem {
    productId: number;
    productName: string;
    stockQuantity: number;
    minStock: number;
    unitType: string;
    status: LowStockItemStatus;
}
export interface SalesReport {
    totalRevenue: number;
    totalSales: number;
    totalDiscount: number;
    avgTicket: number;
    sales: Sale[];
}
export interface ProductReport {
    productId: number;
    productName: string;
    categoryName?: string | null;
    costPrice?: number | null;
    salePrice: number;
    marginPercentage?: number | null;
    quantitySold: number;
    revenue: number;
    estimatedProfit: number;
}
export type ListCategoriesParams = {
    active?: boolean;
};
export type ListProductsParams = {
    search?: string;
    categoryId?: number;
    active?: boolean;
    lowStock?: boolean;
    productType?: ListProductsProductType;
};
export type ListProductsProductType = (typeof ListProductsProductType)[keyof typeof ListProductsProductType];
export declare const ListProductsProductType: {
    readonly simple: "simple";
    readonly composite: "composite";
};
export type UpdateCompositeItemsBody = {
    items: CreateCompositeItemBody[];
};
export type ListSalesParams = {
    from?: string;
    to?: string;
    status?: string;
    userId?: number;
};
export type ListStockParams = {
    lowStock?: boolean;
    search?: string;
};
export type GetDashboardSummaryParams = {
    date?: string;
};
export type GetSalesByHourParams = {
    date?: string;
};
export type GetTopProductsParams = {
    limit?: number;
    from?: string;
    to?: string;
};
export type GetPaymentMethodsSummaryParams = {
    from?: string;
    to?: string;
};
export type GetSalesByCategoryParams = {
    from?: string;
    to?: string;
};
export type GetSalesReportParams = {
    from: string;
    to: string;
    userId?: number;
};
export type GetProductsReportParams = {
    from: string;
    to: string;
};
//# sourceMappingURL=api.schemas.d.ts.map